...
$(PREFIX_DIR)/bin/sdl-config: Tarballs/SDL-1.2.15.tar.gz
	...
	tar -xzf Tarballs/SDL-0b7f217d0dbb.tar.gz -C Build/SDL
	cd Build/SDL/SDL-0b7f217d0dbb && ./configure --prefix="$(PREFIX_DIR)" --without-x
	#patch -p1 < Patches/SDL/001.fix-compilation.patch
	cd Build/SDL/SDL-0b7f217d0dbb && make && make install
